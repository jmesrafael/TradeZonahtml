...
let userIsPro   = false;
let journalSettings = null; // from journal_settings table
let psSettings  = null;     // from presession_settings table
let currentSession = null;  // the active pre_session row
let currentDate = todayLocal();
let activePsTab = 'plan';
let dirtyTabs   = new Set();
let intentDir   = 'Long';
let editingIntentId = null;
let intents     = [];       // trade_intents for today
let sessionHistory = [];    // last N sessions for history bar
...
const FACTORY_TEMPLATES = {
  scalping: {
    ...
    desc: 'Fast-paced intraday entries, tight risk management',
    items: [
      ...
      { text: 'No overtrading after 3 losses',                  cat: 'execution'  },
  ...
  swing: {
    ...
    desc: 'Multi-day holds, higher timeframe confluence',
    items: [
...
function scoreColor(pct, settings) {
  const ok   = settings?.threshOK   ?? 70;
  const good = settings?.threshGood ?? 85;
  if (pct >= good) return 'var(--accent2)';
  if (pct >= ok)   return '#f59e0b';
  return '#ff5f6d';
}
function scoreLabel(pct, settings) {
  const ok   = settings?.threshOK   ?? 70;
  const good = settings?.threshGood ?? 85;
  if (pct >= good) return 'Good to trade';
  if (pct >= ok)   return 'Proceed carefully';
  if (pct === 0)   return 'Complete the checklist below';
  return 'Not ready — review items';
}
...
function showToast(msg, icon='fa-solid fa-circle-check', type='') {
  ...
  document.getElementById('psToastIcon').className = icon;
  document.getElementById('psToastMsg').textContent = msg;
...
(async () => {
  ...
  // Load journal settings (for setup library, moods, etc.)
  journalSettings = await getJournalSettings(jid);

  // Load or create presession_settings
...
async function getPsSettings() {
  const { data, error } = await db
    .from('presession_settings')
    .select('*')
    .eq('journal_id', jid)
    .maybeSingle();

  if (error) console.error('[ps] getPsSettings error:', error);

  if (!data) {
    // Bootstrap with scalping template
    const defaultItems = FACTORY_TEMPLATES.scalping.items;
    const defaultSetups = ['Break & Retest','FVG Fill','Order Block','Trend Continuation','Liquidity Grab','Reversal'];
    const { data: created } = await db
      ...
      .select('*')
      .maybeSingle();
    return created || { checklist_items: defaultItems, setup_library: defaultSetups, feature_toggles:{banner:true,tradeLock:true,autofill:true,reflectReminder:false}, thresholds:{ok:70,good:85}, templates:{} };
  }
  return data;
}
...
async function loadSession(date) {
  // Update header
  document.getElementById('psDateTitle').textContent = 'Pre-Session';
  ...
  if (error) console.error('[ps] loadSession error:', error);

  if (!data) {
    // Auto-create a blank session for this date
    const { data: created } = await db
      .from('pre_sessions')
      .upsert({
        journal_id:      jid,
...
async function loadIntents(date) {
  ...
  const { data, error } = await db
    .from('trade_intents')
    .select('*')
    .eq('pre_session_id', currentSession.id)
    .order('created_at', { ascending: true });
  if (error) console.error('[ps] loadIntents error:', error);
  intents = data || [];
  renderIntents();
}
...
async function loadHistory() {
  const { data } = await db
    .from('pre_sessions')
    .select('session_date, checklist_score')
    .eq('journal_id', jid)
...
function renderBanner() {
  ...
  fill.style.background = scoreColor(score, { threshOK: psSettings?.thresholds?.ok, threshGood: psSettings?.thresholds?.good });
  document.getElementById('sbScoreVal').textContent = score ? score + '%' : '—';

  const phase = currentDate === todayLocal() ? (score > 0 ? 'Session active' : 'Plan your session') : (currentDate < todayLocal() ? 'Past session' : 'Future session');
...
function renderLevels(levels) {
  ...
  (levels || []).forEach((lv, i) => {
    ...
    row.innerHTML = `
      ...
      <input class="level-input" value="${esc(lv.label||'')}" placeholder="Level description…" oninput="updateLevel(${i},'label',this.value)">
      ...
      <button class="level-rm" onclick="removeLevel(${i})"><i class="fa-solid fa-xmark"></i></button>
    `;
...
function addLevel() {
  if (!currentSession) return;
  const levels = currentSession.key_levels || [];
  levels.push({ tag: 'S', label: '', price: '' });
  currentSession.key_levels = levels;
  renderLevels(levels);
  markDirty('plan');
}
function removeLevel(i) {
  currentSession.key_levels.splice(i, 1);
  renderLevels(currentSession.key_levels);
  markDirty('plan');
}
function updateLevel(i, field, val) {
  if (!currentSession.key_levels[i]) return;
  currentSession.key_levels[i][field] = val;
  markDirty('plan');
}
function setLevelTag(i, tag) {
  if (!currentSession.key_levels[i]) return;
  currentSession.key_levels[i].tag = tag;
  renderLevels(currentSession.key_levels);
  markDirty('plan');
}
...
function renderRules(rules) {
  ...
  (rules || []).forEach((r, i) => {
    ...
    row.innerHTML = `
      <input class="rule-input" value="${esc(r.text||'')}" placeholder="e.g. No trading below 3-star mood…" oninput="updateRule(${i},this.value)">
      <button class="rule-rm" onclick="removeRule(${i})"><i class="fa-solid fa-xmark"></i></button>
    `;
    list.appendChild(row);
  });
}
function addRule() {
  ...
  rules.push({ text: '', broken: false });
  currentSession.rules = rules;
  renderRules(rules);
  markDirty('plan');
}
function removeRule(i) {
  currentSession.rules.splice(i, 1);
  renderRules(currentSession.rules);
  markDirty('plan');
}
function updateRule(i, val) {
  if (!currentSession.rules[i]) return;
  currentSession.rules[i].text = val;
  markDirty('plan');
}
...
async function savePlan() {
  ...
  try {
    await upsertSession({
      bias:          currentSession.bias,
      bias_reason:   document.getElementById('biasReason').value,
      session_goals: document.getElementById('sessionGoals').value,
      key_levels:    currentSession.key_levels || [],
      rules:         currentSession.rules || []
    });
    clearDirty('plan');
    renderBanner();
    renderRulesBroken();
...
function renderChecklistItems() {
  ...
  const el     = document.getElementById('clItems');
  ...
  cats.forEach(cat => {
    ...
    lbl.className = 'cl-category-label';
    lbl.textContent = catLabels[cat] || cat;
    el.appendChild(lbl);

    catItems.forEach((item, _) => {
      const globalIdx = items.indexOf(item);
      const checked   = !!state[globalIdx];
      const div = document.createElement('div');
      div.className  = 'cl-item' + (checked ? ' checked' : '');
      div.innerHTML  = `
        <div class="cl-item-check">${checked ? '<i class="fa-solid fa-check"></i>' : ''}</div>
        <div class="cl-item-text">${esc(item.text)}</div>
        <span class="cl-item-cat">${esc(item.cat)}</span>
      `;
      div.addEventListener('click', () => toggleChecklistItem(globalIdx));
      el.appendChild(div);
    });
  });
}
...
function updateChecklistScore() {
  ...
  const col    = scoreColor(pct, { threshOK: psSettings?.thresholds?.ok, threshGood: psSettings?.thresholds?.good });
  if (fill) { fill.style.strokeDashoffset = offset; fill.style.stroke = col; }

  document.getElementById('clScorePct').textContent    = pct ? pct + '%' : '—';
  document.getElementById('clScoreStatus').textContent = scoreLabel(pct, { threshOK: psSettings?.thresholds?.ok, threshGood: psSettings?.thresholds?.good });
  document.getElementById('clScoreCounts').textContent  = total ? `${done} / ${total} items completed` : '';

  const qBar = document.getElementById('clQualityBar');
  if (qBar) {
    const label = pct >= (psSettings?.thresholds?.good || 85) ? 'Good' : pct >= (psSettings?.thresholds?.ok || 70) ? 'OK' : pct > 0 ? 'Poor' : '—';
    qBar.textContent = label;
    qBar.style.color = col;
    qBar.style.borderColor = col + '55';
  }
...
async function saveChecklist() {
  ...
  try {
    ...
    await upsertSession({
      ...
      checklist_score:    pct,
      checklist_snapshot: JSON.stringify(items)
    });
    clearDirty('checklist');
    renderBanner();
    await loadHistory();
...
function renderHistory() {
  const el = document.getElementById('clHistoryBars');
  if (!el) return;
  const rows = sessionHistory.filter(s => s.checklist_score > 0);
  if (!rows.length) {
    el.innerHTML = '<div class="cl-history-empty">Complete a few sessions to see history</div>';
    return;
  }
  el.innerHTML = '';
  rows.slice(-8).forEach(s => {
    ...
    const row = document.createElement('div');
    row.className = 'cl-history-bar-row';
    row.innerHTML = `
      <span class="cl-hbar-date">${m}/${d}</span>
      <div class="cl-hbar-wrap"><div class="cl-hbar-fill" style="width:${pct}%;background:${col}"></div></div>
      <span class="cl-hbar-pct">${pct}%</span>
    `;
    el.appendChild(row);
  });
}
...
function renderIntentLockBar() {
  ...
  const score = currentSession.checklist_score || 0;
  const ok    = psSettings?.thresholds?.ok   || 70;
  const good  = psSettings?.thresholds?.good || 85;
  const toggles = psSettings?.feature_toggles || {};

  if (!toggles.tradeLock) { bar.style.display = 'none'; return; }
  bar.style.display = '';

  let cls, icon, title, sub, badge;
  if (score === 0) {
    cls = 'lock-red'; icon = 'fa-solid fa-triangle-exclamation';
    title = 'Checklist not completed'; sub = 'Complete your pre-session checklist before trading';
    badge = 'Not ready';
  } else if (score < ok) {
    cls = 'lock-red'; icon = 'fa-solid fa-shield-halved';
    title = 'Checklist score is low'; sub = `Score: ${score}% — consider reviewing your setup`;
    badge = score + '% — Poor';
  } else if (score < good) {
    cls = 'lock-amber'; icon = 'fa-solid fa-triangle-exclamation';
    title = 'Proceed with caution'; sub = `Score: ${score}% — trade carefully`;
    badge = score + '% — OK';
  } else {
    cls = 'lock-green'; icon = 'fa-solid fa-shield-halved';
    title = 'Ready to trade'; sub = `Checklist score: ${score}% — conditions met`;
    badge = score + '% — Good';
  }
...
function renderIntents() {
  ...
  intents.forEach(intent => {
    ...
    const score = currentSession?.checklist_score || 0;
    const col   = scoreColor(score, { threshOK: psSettings?.thresholds?.ok, threshGood: psSettings?.thresholds?.good });
    ...
    card.innerHTML = `
      ...
      <div class="intent-card-body" id="icb_${intent.id}">
        <div class="icb-row">
...
function toggleIntentCard(id) {
  const body = document.getElementById('icb_' + id);
...
function calcIntentRR() {
  const entry = parseFloat(document.getElementById('ifEntry').value);
  const sl    = parseFloat(document.getElementById('ifSL').value);
  const tp    = parseFloat(document.getElementById('ifTP').value);
...
function newIntent() {
  ...
  document.getElementById('ifSetup').value      = '';
  document.getElementById('ifWhy').value        = '';
  document.getElementById('ifEntry').value      = '';
  document.getElementById('ifSL').value         = '';
  document.getElementById('ifTP').value         = '';
  document.getElementById('ifInvalidation').value = '';
  document.getElementById('ifRRDisplay').style.display = 'none';
  setIntentDir('Long');
  populateSetupSelect();
...
function editIntent(id) {
  ...
  intentDir = intent.direction || 'Long';
  setIntentDir(intentDir);
...
function cancelIntent() {
  editingIntentId = null;
  document.getElementById('intentFormWrap').style.display = 'none';
}
...
async function saveIntent() {
  ...
  const payload = {
    pre_session_id:      currentSession.id,
    journal_id:          jid,
    user_id:             currentUser.id,
    setup_name:          setup,
    direction:           intentDir,
...
async function deleteIntent(id) {
  ...
  try {
    ...
    showToast('Delete failed: ' + e.message, 'fa-solid fa-circle-exclamation', 'red');
  }
}
...
function renderReflectMoods(selected) {
  ...
  const row    = document.getElementById('reflectMoodRow');
  if (!row) return;
  row.innerHTML = '';
  moods.forEach(m => {
    ...
    pill.style.cssText = selected === m
      ? `background:rgba(${r},${g},${b},.18);color:${col};border-color:rgba(${r},${g},${b},.5)`
      : '';
    pill.textContent = m;
    pill.addEventListener('click', () => {
      currentSession.reflect_mood = currentSession.reflect_mood === m ? null : m;
      renderReflectMoods(currentSession.reflect_mood);
      markDirty('reflect');
    });
...
function renderRulesBroken() {
  ...
  if (!rules.length) {
    list.innerHTML = '<div style="font-size:12px;color:var(--muted)">Save your plan rules first, then mark any you broke.</div>';
    return;
  }
  list.innerHTML = '';
  rules.forEach((r, i) => {
    ...
    row.innerHTML = `
      <div class="rbr-check">${isBroken ? '<i class="fa-solid fa-xmark" style="font-size:8px"></i>' : ''}</div>
      <div class="rbr-text">${esc(r.text)}</div>
    `;
    row.addEventListener('click', () => {
      const rb = currentSession.rules_broken || [];
      const idx = rb.indexOf(i);
      if (idx > -1) rb.splice(idx, 1); else rb.push(i);
      currentSession.rules_broken = rb;
      renderRulesBroken();
      markDirty('reflect');
    });
    list.appendChild(row);
...
function renderAutoInsights() {
  ...
  if (sessionHistory.length < 5) {
    el.innerHTML = `<div class="ai-placeholder"><i class="fa-solid fa-chart-simple" style="font-size:22px;opacity:.2;margin-bottom:8px;display:block"></i>Build up ${Math.max(0,5-sessionHistory.length)} more session${5-sessionHistory.length!==1?'s':''} to unlock behavioral pattern insights.</div>`;
    return;
  }
  const insights = [];
  const scores = sessionHistory.map(s => s.checklist_score || 0);
  const avg    = Math.round(scores.reduce((a,b)=>a+b,0) / scores.length);
  insights.push({ icon:'fa-solid fa-chart-line', text:`Your average checklist score over the last ${sessionHistory.length} sessions is <strong>${avg}%</strong>.` });

  el.innerHTML = insights.map(i => `
    <div class="ai-insight"><i class="${i.icon}"></i><div class="ai-insight-text">${i.text}</div></div>
  `).join('');
}
...
function renderTemplateRow() {
  const el     = document.getElementById('czTemplateRow');
  ...
  Object.entries(custom).forEach(([key, t]) => {
    const btn = document.createElement('button');
    btn.className = 'cz-template-btn' + (active === key ? ' active' : '');
    btn.innerHTML = `<i class="fa-solid fa-star"></i> ${esc(t.name)} <button class="cz-rm" onclick="event.stopPropagation();deleteTemplate('${key}')"><i class="fa-solid fa-xmark"></i></button>`;
...
function renderTemplateGrid() {
  ...
  Object.entries(FACTORY_TEMPLATES).forEach(([key, t]) => {
    ...
    div.addEventListener('click', () => { applyTemplate(key); closeTemplateModal(); });
    grid.appendChild(div);
  });

  Object.entries(custom).forEach(([key, t]) => {
    const div = document.createElement('div');
    div.className = 'template-opt topt-user' + (active === key ? ' active' : '');
...
async function applyTemplate(key, items) {
  const template = FACTORY_TEMPLATES[key] || { items };
  const confirmed = confirm(`Apply "${FACTORY_TEMPLATES[key]?.name || key}" template? This replaces your current checklist items.`);
  if (!confirmed) return;
  try {
    await savePsSettings({ checklist_items: template.items, active_template: key });
    renderCzItems();
    renderChecklistItems();
    updateChecklistScore();
    renderTemplateRow();
...
async function deleteTemplate(key) {
  ...
  const templates = { ...(psSettings?.templates || {}) };
  delete templates[key];
  try {
    await savePsSettings({ templates });
    renderTemplateRow();
    showToast('Template deleted.', 'fa-solid fa-circle-check', 'green');
  } catch(e) {
    showToast('Error: ' + e.message, 'fa-solid fa-circle-exclamation', 'red');
  }
}
...
function renderCzItems() {
  ...
  const el    = document.getElementById('czItemsList');
  el.innerHTML = '';
  items.forEach((item, i) => {
    const row = document.createElement('div');
    row.className = 'cz-item-row';
    row.setAttribute('draggable', 'true');
    row.dataset.idx = i;
    row.innerHTML = `
      <span class="cz-item-drag"><i class="fa-solid fa-grip-vertical"></i></span>
      <span class="cz-item-text" contenteditable="true" onblur="updateCzItem(${i},this.textContent)">${esc(item.text)}</span>
      <span class="cz-item-cat-badge">${esc(item.cat)}</span>
      <button class="cz-item-rm" onclick="removeCzItem(${i})"><i class="fa-solid fa-xmark"></i></button>
    `;
    // Drag to reorder
    ...
    row.addEventListener('drop', async e => {
      ...
      if (from === to) return;
      const items2 = [...(psSettings?.checklist_items || [])];
      const [moved] = items2.splice(from, 1);
      items2.splice(to, 0, moved);
      await savePsSettings({ checklist_items: items2 });
      renderCzItems();
      renderChecklistItems();
    });
    el.appendChild(row);
...
async function removeCzItem(i) {
  ...
  try {
    ...
    renderChecklistItems();
    updateChecklistScore();
    showToast('Item removed.', 'fa-solid fa-circle-check', 'green');
  } catch(e) {
    showToast('Error: ' + e.message, 'fa-solid fa-circle-exclamation', 'red');
  }
}
...
async function addChecklistItem() {
  ...
  try {
    await savePsSettings({ checklist_items: items });
    document.getElementById('czNewItem').value = '';
    renderCzItems();
    renderChecklistItems();
    updateChecklistScore();
    showToast('Item added!', 'fa-solid fa-circle-check', 'green');
  } catch(e) {
    showToast('Error: ' + e.message, 'fa-solid fa-circle-exclamation', 'red');
  }
}
...
function renderSetupList() {
  const setups = psSettings?.setup_library || [];
  const el     = document.getElementById('czSetupList');
  el.innerHTML = '';
  setups.forEach((s, i) => {
    const tag = document.createElement('div');
    tag.className = 'cz-setup-tag';
...
async function addSetup() {
  const val = document.getElementById('czNewSetup').value.trim();
  if (!val) return;
  const setups = [...(psSettings?.setup_library || [])];
  ...
  setups.push(val);
  try {
    await savePsSettings({ setup_library: setups });
    document.getElementById('czNewSetup').value = '';
    renderSetupList();
    showToast('Setup added!','fa-solid fa-circle-check','green');
  } catch(e) {
    showToast('Error: ' + e.message,'fa-solid fa-circle-exclamation','red');
  }
}
...
async function removeSetup(i) {
  ...
  try {
    await savePsSettings({ setup_library: setups });
    renderSetupList();
    showToast('Setup removed.','fa-solid fa-circle-check','green');
  } catch(e) {
    showToast('Error: ' + e.message,'fa-solid fa-circle-exclamation','red');
  }
}
...
function czToggle(key) {
  ...
  psSettings.feature_toggles = toggles;
  renderBanner();
}
...
async function saveCustomize() {
  ...
  const good = parseInt(document.getElementById('threshGood').value);
  try {
    await savePsSettings({
      feature_toggles: psSettings?.feature_toggles || {},
      thresholds: { ok, good }
    });
    updateChecklistScore();
    renderBanner();
...
// ── Journal settings reload ──────────────────────────────────────────
async function reloadJournalSettings() {
  journalSettings = await getJournalSettings(jid);
  renderReflectMoods(currentSession?.reflect_mood);
}
